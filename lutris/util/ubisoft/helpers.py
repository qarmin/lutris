...
def _get_registry_value_from_path(registry_path, key) -> str:
    registry = WineRegistry()
    return registry.query(registry_path, key)


def _return_local_game_path_from_special_registry(special_registry_path) -> str:
...
def _return_local_game_path(launch_id) -> str:
...
def get_local_game_path(special_registry_path, launch_id) -> str:
...
def _is_file_at_path(path, file) -> bool:
    if os.path.isdir(path):
        return os.path.isfile(os.path.join(path, file))
    return False


def _read_status_from_state_file(game_path) -> str:
    try:
...
def get_game_installed_status(path: Optional[str], exe:Optional[str]=None, special_registry_path=None) -> str:
...
def get_ubisoft_registry(prefix, fullpath: Optional[str]) -> str:
